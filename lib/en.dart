//Homescreen text

const String WELCOME = "Welcome";
const String USER = "User";
const String HOME = "Home";
const String RETRY = "Retry";
const String SEE_ALL = "see all";
const String UPCOMING_APPOINTMENTS = "Upcoming Bookings";
const String SEARCH_DOCTOR_BY_NAME = "Search for location";
const String NEARBY_DOCTORS = "Nearby Guides";
const String TURN_ON_LOCATION_AND_RETRY = "Turn on location and retry";

const String ALL_APPOINTMENTS = "All Bookings";
const String DOCTOR_DETAILS = "Guide detail";
const String SEE_ALL_REVIEW = "See all reviews";
const String PHONE_NUMBER = "Phone number";
const String PHONE = "Phone";
//const String EMAIL = "Email";
const String ABOUT_US = "'About me'";
const String ADDRESS = "Address";
const String WORKING_TIME = "Working time";
const String WORKING_TIME_LIMIT = "Working Time should be within 9.00 - 17.00";
const String WORKING_TIME_FORMAT = "Format __ - __";
const String SERVICES = "Activities";
const String MAKE_AN_APPOINTMENT = "Make a booking";
const String MAKE_AN_SUBSCRIBE = "Make a subscribe";
const String LOGIN_TO_BOOK_APPOINTMENT = "Login to book now";
const String DOCTOR_DASHBOARD = "Guide dashboard";
const String TOUR = "Trips";
const String INBOX = "Inbox";
const String APPOINTMENTS = "Bookings";
const String EITHER_MOBILE_NUMBER_OR_PASSWORD_IS_INCORRECT =
    "Either mobile number or password is incorrect";
const String EITHER_EMAIL_OR_PASSWORD_IS_INCORRECT =
    "Either email or password is incorrect";
const String LOGIN = "Login";
const String DO_NOT_HAVE_AN_ACCOUNT = "Don't have an account?";
const String REGISTER_NOW = "Register Now";
const String ENTER_YOUR_EMAIL = "Enter your email";
const String ENTER_VALID_EMAIL_ADDRESS = "Enter valid email address";
const String PASSWORD = "Password";
const String LOGGING_IN = "Logging In";
const String PLEASE_WAIT_LOGGING_IN = "Please wait while logging in...";
const String OK = "Ok";
const String ENTER_MOBILE_NUMBER = "Enter mobile number";
const String ENTER_VALID_MOBILE_NUMBER =
    "Enter valid mobile number must be length of 11 digits";
const String SUNDAY = "Sun";
const String MONDAY = "Mon";
const String TUESDAY = "Tue";
const String WEDNESDAY = "Wed";
const String THURSDAY = "Thu";
const String FRIDAY = "Fri";
const String SATURDAY = "Sat";
const String NO_SLOT_AVAILABLE = "No slot available";
const String MORE = "Tourist Login";
const String ERROR = "Error!";
const String REGISTER_AS_DOCTOR = "Register as guide";
const String REGISTER = "Register";
const String ALREADY_HAVE_AN_ACCOUNT = "Already have an account?";
const String LOGIN_NOW = "Login Now";
const String ENTER_NAME = "Enter Name";
const String PASSWORD_DOES_NOT_MATCH = "Password doesn't match";
const String CONFIRM_PASSWORD = "Confirm Password";
const String NEW_PASSWORD = "New Password";
const String OLD_PASSWORD = "Old Password";
const String CREATING_ACCOUNT = "Creating Account";
const String PLEASE_WAIT_WHILE_CREATING_ACCOUNT =
    "Please wait while creating account...";
const String REVIEW = "Review";
const String ADD_A_REVIEW = "Add a review";
const String SUBMIT = "Submit";
const String YOUR_RATING = "Your rating";
const String ENTER_YOUR_MESSAGE = "Enter your message";
const String UPLOADING_REVIEW = "Uploading review";
const String WAIT_FOR_A_WHILE = "Wait for a while...";
const String SEARCH = "search";
const String HERE = "Here";
const String SPECIALITY = "Speciality";
const String DESCRIPTION = "Description";
const String PROCESSING = "Processing";
const String SUCCESSFUL = "Successful";
const String SUCCESSFUL_TIME_SLOT = "Time Slot Added Successfully";
const String APPOINTMENT_MADE_SUCCESSFULLY = "Booking made successfully";
const String PLEASE_WAIT_WHILE_MAKING_APPOINTMENT =
    "Please wait while making the booking";

//DoctorAppointmentDetails
const String APPOINTMENT = "Booking";
const String ACCEPT = "Accept";
const String CANCEL = "Cancel";
const String EMAIL_ADDRESS = "Email Address";

//DoctorProfile
const String PROFILE = "Profile";
const String NEXT = "Next";
const String NAME = "Name";
const String ENTER_WORKING_TIME = "Enter working time";
const String ENTER_ABOUT_US = "Enter about us";
const String ENTER_SERVICES = "Enter services";
const String START_TIME = "Start time";
const String END_TIME = "End time";
const String SAVE = "Save";
const String AM = "AM";
const String PM = "PM";
const String PLEASE_WAIT_WHILE_SAVING_CHANGES =
    "Please wait while saving changes";
const String MINUTES = "Min";
const String CONTINUE = "Continue";
const String EMPTY = "Empty";
const String NO_CHANGES_HAVE_BEEN_MADE = "No changes have been made";
const String ENDING_TIME_CAN_NOT_BE_SMALLER_THAT_STARTING_TIME =
    "Ending time can't be smaller than starting time";
const String TIMINGS_ENTERED_INCORRECTLY = "Timings entered incorrectly";
const String HEALTHCARE = "Languages";
const String FACEBOOK = "Facebook Url";
const String TWITTER = "Twitter Url";
const String SELECT_DEPARTMENT = "Select department";

//Morescreen
const String NAME_PAGE = "Use your real first name";
const String ABOUT = "About Me";
const String LOCATION = "Location";
const String EMAIL_PAGE =
    "Your email address will be used to send you booking confirmations. It won't be displayed on your profile.";
const String ABOUT_ME_COL1 =
    "Let others know what you do in life and how you spending your free time.";
const String ABOUT_ME_COL2 =
    "Important: We love all languages, but kindly ask to fill out this section in English. Please avoid writing brands, company names and links to other sites in your description.";
const String SPECIALIST = "Specialist";
const String TERM_AND_CONDITIONS = "Term & Conditions";
const String HELP_CENTER = "Help Center";
const String REPORT_ISSUES = "Report Issues";
const String LOGOUT = "Logout";
const String UPDATE = "Update";
const String SIGN_IN = "Sign In";
const String SIGN_IN_TO_CONTINUE = "Sign in to continue";
const String REPORT = "Report";
const String YES = "Yes";
const String NO = "No";
const String COMPLETE = "Complete";
const String ABSENT = "Absent";
const String DENTIST = "Dentist";
const String REVIEWS = "reviews";
const String ENTER_PASSWORD = "Enter password";
const String PROFILE_UPDATED_SUCCESSFULLY = "Profile updated successfully";
const String LOGIN_TO_ADD_REVIEW = "Login to add review";
const String No_REVIEWS = "No reviews";
const String SELECT_ADDRESS_FROM_MAP = "Select address from map";
const String START_END_TIME_NOT_ENTERED_CORRECTLY =
    "Start/End time not entered correctly";
const String ARE_YOU_SURE_TO_LOGOUT = "Are you sure to logout";
const String PLEASE_WAIT_WHILE_REPORTING_ISSUE =
    "Please wait while reporting issue...";
const String PLEASE_WAIT_WHILE_PROCESSING = "Please wait while processing...";
const String YOU_DONOT_HAVE_ANY_UPCOMING_APPOINTMENT =
    "You don't have any booking";
const String FIND_BEST_DOCTORS_NEAR_YOU_BY_SPECIALITY =
    "Find best guides near you by speciality,";
const String CLICK_HERE = "click here";
const String THIS_FIELD_IS_REQUIRED = "This field is required";
const String ADDRESS_GOES_HERE = "Address goes here...";
const String GUIDE_LOGIN = "Locals Login";
const String HEALTH_CARE = "Languages";
const String ABOUT_GUIDE = "About Me";
const String I_WILL_SHOW_YOU = 'I will show you';
const String LOADING = "Loading...";
const String CONTINUE_WITH_GOOGLE = "Continue with google";
const String CONTINUE_WITH_APPLE_ID = "Continue with apple id";
const String CONTINUE_WITH_FACEBOOK = "Continue with facebook";
const String PERMISSION_NOT_GRANTED = "Permission not granted";
const String PLEASE_WAIT_WHILE_LOGGING_INTO_ACCOUNT =
    "Please wait while logging into account...";
String ENTER_THE_EMAIL_ADDRESS_ASSOCIATED_WITH_YOUR_ACCOUNT =
    "Enter the email address associated with your account";
String WE_WILL_EMAIL_YOU_A_LINK_TO_RESET_YOUR_PASSWORD =
    "We will email you a link to reset you password";
String FORGET_PASSWORD = "Forget Password ?";
String PLEASE_SELECT_APPOINTMENT_TIME = "Please select appointment time";
String UNABLE_TO_LOAD_DATA_FORM_SERVER =
    "Something went wrong, Unable to load data";
String UNABLE_TO_SAVE_TOKEN_TO_SERVER = "Unable to save token to server";

String DOCTOR_NOT_TAKING_APPOINTMENT_ON_TIME =
    "Guide not taking appointments on time";
String DOCTOR_IS_SPAM_AND_DETAILS_ARE_INCORRECT =
    "Guide is spam and details are incorrect";
String ADDRESS_DETAILS_ARE_INCORRECT = "Address details are incorrect";
String DOCTOR_NOT_TREATING_WELL = "Guides not treating well";
String
    DOCTOR_ACCEPTED_APPOINTMENT_BUT_WHEN_I_VISIT_ASSISTANT_DIAGONISING_PATIENTS =
    "Guide accepted appointment but when i visit assistant diagnosing patinets";

String JANUARY = "Jan";
String FEBRUARY = "Feb";
String MARCH = "March";
String APRIL = "April";
String MAY = "May";
String JUNE = "June";
String JULY = "July";
String AUGUST = "Aug";
String SEPTEMBER = "Sep";
String OCTOBER = "Oct";
String NOVEMBER = "Nov";
String DECEMBER = "Dec";

String RECEIVED = "Received";
String APPROVED = "Approved";
String IN_PROCESS = "In Process";
String COMPLETED = "Completed";
String REJECTED = "Rejected";

String INVALID_DETAILS = "Invalid Details";
String ALERT = "Alert!";
String BOOK_NOW = "Book Now";

String CONSULTATION_FEE = "Consultation fee";

String WAIT = "wait";
String PROCESS_PAYMENT = "Process Payment";
String NOT_SPECIFIED = "Not specified";
String ARE_YOU_SURE_TO_CONTINUE = "Are you sure to continue?";
String YOU_WILL_CHARGED_CONSULTAION_FEE_OF =
    "You will be charged consultation fee of ";
String CAN_NOT_MAKE_APPOINTMENT = "Can't make an appointment";
String APPOINTMENT_CAN_NOT_BE_MADE_AS_COSULTAION_FEE_IS_NOT_SPECIFIED =
    "Appointment can't made as consultation fee is not specified by the guide";

String BRAINTREE = "Braintree";
String BRAINTREE_EXPLANATION =
    "use your credit/debit card or PayPal account to pay consultation fee";

String STRIPE = "Stripe";
String STRIPE_EXPLANATION =
    "use your credit/debit card to pay consultation fee";

String RazorpayString = "Razorpay";
String RAZORPAY_EXPLANATION =
    "use your credit/debit card to pay consultation fee";

String PAYSTACK = "Paystack";
String RAVE = "Rave";
String PAYTM = "Paytm";

String FAIL = "Fail";
String FAIL_DES = "Your payment fail try again";

String ONLINE = "Online";
String ONLINE_EXPLAIN = "use your credit/debit card to pay Subscription fee";
String ONLINE_EXPLAIN_BOOKING =
    "use your credit/debit card to pay appointment fee";

String SELECT_A_PAYMENT_METHOD = "Select payment method";

String APPOINTMENT_FEE = "Appointment Fee";
String CASH_ON_BOARD = "Cash On Board";
String PAY_ON_VISTING_DOCTOR =
    "Pay only on visiting guide at your appointment time";

String UPLOAD_PRESCRIPTION = "Upload Prescription";
String PRESCRIPTION = "Prescription";
String REMOVE_PRESCRIPTION = "Remove Prescription";
String UPLOAD_PRESCRIPTION_DESC =
    "In order to mark this this appointment as complete, you need to upload prescription first.";

String ADD_HOLIDAY = "Add Holiday";
String UPDATE_HOLIDAY = "Update Holiday";
String REMOVE = "Remove";
String ADD_DATE_ERROR = "Please select a date to add it in your holiday list";

String NO_HOLIDAY_SO_FAR = 'No holidays so far';
String NO_HOLIDAY_SO_FAR_DESC = 'Your holiday list will appear here';
String DOCTOR_ON_LEAVE = 'Guide on leave';
String DOCTOR_ON_LEAVE_DESC(String date) =>
    'Not possible to book appointment on $date as guide in on leave';
String UPLOAD_RECEIPT = 'Upload Receipt';
String UPLOAD_RECIPES_DES = 'Upload Payment Receipt';
String CHOOSE_YOUR_PLAN = 'CHOOSE YOUR PLAN';
String HELLO_DR = 'Hello ';
String Monthly = 'Monthly';
String Quarterly = 'Quarterly';
String HalfYear = 'HalfYear';
String Yearly = 'Yearly';
String UPLOAD_RECIPE = 'Upload Recipe';
String UPLOAD_RECIPE_DES = 'Upload Recipe Description and dummy text';
String ENTER_DES = 'Enter Description';
String Dummy1 = 'Lorem ipsum simply dummy text';
String Dummy2 = 'Long established fact';
String CHOOSE_BTN = 'Choose plan >';
String UPLOAD_RECIPES = 'Upload Recipes';
const String MORE_INFO = "Account Information";
// String UPLOAD_RECIPES_DES = 'Upload Payment Recipe';
String RECIPES_UPLOADING = 'Recipes Uploading...';
String SOMETHING_WRONG = 'Something Wrong';
String PRESCRIPTION_UPLOADING = 'Prescription Uploading...';
String ENTER_DESCRIPTION = 'Enter Description';
String SavePercentage = 'Save 50%';
String Gett = 'Get';
String Month = 'Month';
String Days = ' 7 days';
String Free = ' free';
String Available = '24/7 available';
String LongText =
    'Guide can purchase subscription and start using our app, after subscribe you can start dealing with your appointment, complete your profiles, set time slots, you can also check your reviews by users, and many more features';
const String PLEASE_TRY_AGAIN = "Please try again";
const String PASSWORD_STR = "Password";
const String AWAITING_DOCTOR_ACCEPTANCE = "Awaiting Guide Acceptance";
const String CALL_ACCEPT = "Call Accepting";
const String PLEASE_WAIT_CALL_ACCEPT = "Please wait your call is accepting...";
const String APPOINTMENT_REMAINING_TIME = "Appointment Remaining Time";
const String EDIT_PROFILE = "Edit Profile";
const String GENERAL_INFORMATION = 'General information';
const String CONTACT_AND_IDENTIFICATION = 'Contact & identification';
const String MEMBERSHIP = 'Membership';
const String ABOUT_HOST = 'About host';
const String CURRENCY_EXCHANGE = 'Currency';
const String NOTIFICATION_SETTINGS = 'Notification settings';
const String DELETE_ACCOUNT = 'Delete Account';
const String CURRENT_DATE = 'Select a date';
const String DATE_FROM = "Select a date from";
const String DATE_TO = "Select a date to";
const String PEOPLE = 'Number of people';
const String SUGGESTION = "Any Thing Special You Like to Tell?";
const String LOOKING_LOCAL = 'Looking for a local';
const String RECENT_CHATS = "Recent chats";
const String CHANGE_PASSWORD = "Change Password";
const String DELETE_MESSAGE =
    "After deleting account you can not able to login untill you resgister again!";
const String SUBSCRIPTION = "Subscription";
const String INCOME_REPORT = "Income Report";
const String DETAILED_REPORTS = "Detailed Reports";
const String YOU_HAVE_NOT_ANY_INCOME = "you have not any income.";
const String PROVIDE_YOUR_MOBILE_NUMBER = "Provide your Mobile number";
const String WHAT_BRING_YOU_HERE = "ANYTHING SPECIFIC YOU WOULD LIKE TO SEE?";
