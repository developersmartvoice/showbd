...
import 'package:appcode3/modals/holiday_model.dart';
...
class _HolidayListState extends State<HolidayList> {
    ...
    return Scaffold(
      ...
      floatingActionButton: Padding(
        ...
        child: FloatingActionButton(
          ...
          onPressed: () async {
            bool x = await Navigator.push(context,
                MaterialPageRoute(builder: (context) => AddUpdateHoliday()));
            if (x != null) {
              setState(() {
                futureHoliday = loadHolidays();
              });
  ...
  holidayCard(
      int id, DateTime startDate, DateTime endDate, String description) {
    return Container(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      (startDate.difference(endDate).inDays.abs() + 1)
                              .toString() +
                          ' Days Holiday',
                      style: TextStyle(
  ...
  Future<HolidayModel> loadHolidays() async {
    var request = http.Request(
        'GET', Uri.parse('$SERVER_ADDRESS/api/getholiday?doctor_id=$doctorId'));
